...
SELECT
	(SELECT SUM(target_id) FROM likes WHERE user_id = profiles.user_id AND profiles.gender = 'P' ORDER BY target_id DESC) AS P, -- сумма лайков женщин
	(SELECT SUM(target_id) FROM likes WHERE user_id = profiles.user_id AND profiles.gender = 'M' ORDER BY target_id DESC) AS M, -- сумма лайков мужчин
	(SELECT SUM(target_id) FROM likes WHERE user_id = profiles.user_id AND profiles.gender = 'D' ORDER BY target_id DESC) AS D, -- сумма лайков оно
	gender
FROM profiles GROUP BY user_id; -- больше всего лайков поставили мужчины
...
SELECT
	(SELECT SUM(target_id) FROM likes WHERE user_id = profiles.user_id ORDER BY target_id DESC) AS likes,
	birthday
FROM profiles ORDER BY birthday = 0, birthday DESC LIMIT 10;
...
SELECT 
	user_id, 
	target_id AS likes,
	(SELECT COUNT(id) FROM posts WHERE user_id = likes.user_id GROUP BY user_id ORDER BY COUNT(id)) AS posts 
FROM likes ORDER BY target_id
LIMIT 10;
